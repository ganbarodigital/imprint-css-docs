...
ImprintCSS ships with a large (and thoughly documented!) choice of color palettes right out of the box.
...
<p className="p--leader">ImprintCSS uses the spelling 'gray' everywhere (except for when we've lapsed back into British English by mistake!)</p>
...
In computing, _gray_ is the accepted spelling. ([Wikipedia says that this stems from the X11 color names list](https://en.wikipedia.org/wiki/X11_color_names)). That's why ImprintCSS uses _gray_ and not _grey_.
